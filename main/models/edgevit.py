...
class EdgeVit(nn.Module):
    ...
    """
    def __init__(self, depth=[1, 2, 5, 3], img_size=256, in_chans=3, num_classes=1000, embed_dim=[48, 96, 240, 384],
                 head_dim=64, mlp_ratio=4., qkv_bias=True, qk_scale=None, representation_size=None,
                 drop_rate=0., attn_drop_rate=0., drop_path_rate=0., norm_layer=None,get_feat: bool = False, sr_ratios=[4,2,2,1], **kwargs):
        """
        Args:
...
def get_blocks_to_drop():
    blocks=['blocks1.0',
    'blocks2.0','blocks2.1',
    'blocks3.0','blocks3.1','blocks3.2','blocks3.3','blocks3.4',
    'blocks4.0','blocks4.1','blocks4.2']
    return blocks
def edge_rm_blocks(rm_blocks: list = [], num_classes=100,pretrained: bool = False):
    #rm_blocks: 'blocks1.0'
    blocks=[1,2,5,3]
    for block in rm_blocks:
        layer_num=int(block[6])
        blocks[layer_num-1]-=1
    print(blocks)
    model=EdgeVit(
        depth=blocks,
        embed_dim=[48, 96, 240, 384], head_dim=48, mlp_ratio=[4]*4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), sr_ratios=[4,2,2,1],num_classes=num_classes)
    if pretrained:
        path_checkpoint = "./edgevit_best.pth"
        checkpoint = torch.load(path_checkpoint,map_location='cuda:0')
        state_dict=checkpoint['net']
        load_rm_block_state_dict(model, state_dict, rm_blocks)
    return model
def edgevit(pretrained=False,num_classes=22):
    model=EdgeVit(
        depth=[1, 2, 5, 3],
        embed_dim=[48, 96, 240, 384], head_dim=48, mlp_ratio=[4]*4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), sr_ratios=[4,2,2,1], num_classes=num_classes)
    if pretrained:
        path_checkpoint = "./edgevit_best.pth"  # 断点路径
        checkpoint = torch.load(path_checkpoint,map_location='cuda:0')
        state_dict=checkpoint['net']
        model.load_state_dict(state_dict)
        print('load teacher model')
    return model
def load_rm_block_state_dict(model, raw_state_dict, rm_blocks):#第二个参数是训练好的模型的dict
#'blocks1.0'
    ...
    for raw_key in raw_state_dict.keys():
        items = raw_key.split('.')
        layer =items[0]
        if 'blocks' not in layer:#除了那四个阶段的模块 下采样也在这里
            state_dict[raw_key] = raw_state_dict[raw_key] #赋权重
            continue
        else:
            layer, block = items[0], items[1] #'blocks3'. '0'
            check_rm = "{}.{}".format(layer, block)#blocks3.0
            if check_rm in rm_blocks:#如果原始的被删除了
                if check_rm not in has_count:#并且没有被记录
                    has_count.add(check_rm)
                    layer_rm_count[layer] += 1
            else:
                items[1]=str(int(items[1])-layer_rm_count[layer])#blocks.1-1 把原本的1赋值给现在的0  0被删了 1变0了
                target_key='.'.join(items)#这个.干啥的 这是分隔符
                #print(target_key,raw_key)
...
if __name__ == "__main__":
    ...
    'blocks3.0','blocks3.1','blocks3.2','blocks3.3','blocks3.4',
    'blocks4.0','blocks4.1','blocks4.2']
    model=edgevit(pretrained=False,num_classes=22)
    img=torch.FloatTensor(4, 3, 256, 256)
    pred=model(img)
    print(pred.shape)
    s_model=edge_rm_blocks(rm_blocks,pretrained=False,num_classes=22)
